...
# ╔═╡ 289e4acc-23ae-4551-9ab2-2b0d7a2c4f52
using Images
...
xs = Array{Float32}(undef, 3, 3, 3)

# ╔═╡ 4f2d5791-d2ac-4a47-9bd2-2d10fdf862c9
ts = trues(10)
...
let
	strings = Array{String}(undef, 10)
	fill!(strings, "They're taking the hobbits to Isengard!")
end
...
@benchmark slowfunc($rs)

# ╔═╡ 9eccdb07-b5c3-4285-a1bd-f90f71237558
@benchmark fastfunc($rs)

# ╔═╡ 3f7af6e3-5555-4b20-89ff-bf952a0618d6
@benchmark sum($rs)

# ╔═╡ c9cfa525-37fd-499a-9c3b-ea30e928b816
md"""
!!! exercise
	Conway's game of life evolve a matrix containing only ones (alive cells) and zeros (dead cells) with the following rules.
    ...
    * Any dead cell with exactly three live neighbours becomes a live cell, as if by reproduction.
	...
	Implement the `neighbors` function in the following cell and fix the bug to display the grid evolution.
...
!!! hint
	...
	function neighbors(m, i, j)
		...
		for k in 1:3
			for l in 1:3
				ip = mod(i + k - 2, axes(m, 1))
				jp = mod(j + l - 2, axes(m, 2))
				subm[k, l] = m[ip, jp]
			end
		end
		subm
	...
	```
	The bug is in the last line, you should `copy` the grid!
"""

# ╔═╡ 66bd9ee8-8228-46f8-b140-8863d86fa6fd
let
	T = Int
	N = 32
	p = 1 // 16
	...
	neighbors(m, i, j) = [dead dead dead   # implement me
						  dead alive alive
	                      dead dead dead]
	...
	m = rand([fill(alive, numerator(p)); fill(dead, denominator(p))], N, N)
	map(x -> Gray.(x), [evolve!(m) for _ = 1:4])
end
...
List comprehensions can be created using the `[f(x) for x in xs]` syntax. The comprehension can loop over multiple iterators, in that case a multi-dimensional or a one-dimensional iterator is constructed based on the repetition of the `for` keyword. They can contain also guards (conditions), in presence of such a one-dimensional array is created automatically. The element type can be enforced with the same syntax of array literals.

Omitting the square brackets (or using parenthesis) a generator is instantiated. Generators are iterators that lazily produce elements, without allocating and creating an array to store all the elements.
"""

# ╔═╡ 37067e15-0110-4dc9-8284-994fd659e8cf
[cos(x^2 + y^2) for x in range(-1, 1, length=10), y in range(-1, 1, length=10)]

# ╔═╡ 40e23ea2-3634-44db-8bf8-b3b3002a5822
[cos(x^2 + y^2) for x in range(-1, 1, length=10) for y in range(-1, 1, length=10)]

# ╔═╡ 3253722d-5e7a-4e31-b3dd-ca842c1e7b02
[n for n = 1:50 if (mod(n, 3) == 0 || mod(n, 5) == 0) && mod(n, 15) != 0]

# ╔═╡ 39708ca1-48c5-4499-a299-59c12ffe6c6b
prod("They're taking the hobbits to Isengard! " for _ = 1:10)
...
manifest_format = "2.0"
project_hash = "cef6a0268391adca633a90bab3a9e3b3739de7e9"
