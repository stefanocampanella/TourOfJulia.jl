...
md"""
!!! exercise
	Implement the function `isjuliaset(z0, f)` in the cell below that tests whether the sequence ``z_{n + 1} = f(z_n)`` starting at `z0` escapes a ball centered in ``0.0`` and of radius `R` in at most `max_iterations` steps, returning the number of steps it took normalized between ``0`` and ``1``.

!!! hint
	You can compute the norm of a complex number using the function `norm`. Hence a simple implementation would just be
	```julia
	function isjuliaset(z, f)
		local n
		for outer n = 1:max_iterations
			next_z = f(z)
			norm(next_z) >= R && break
			z = next_z
		end
		(n - 1) / (max_iterations - 1)
	end
	```
"""
...
let	
	...
	rescale(k, N) = R * (2k - N - 1) / (N - 1)

	isjuliaset(z, f) = 0.0 # Replace with your solution!
