...
begin
	...
	for epoch = 1:1_000
		for (x, y) in loader
			...
			Flux.update!(optim, model, only(grads))
			push!(losses, loss)
		end
	end
	losses
...
# ╔═╡ 30083a05-9187-484b-9d11-9d99ac8d1f5a
out = model(noisy)

# ╔═╡ 6184eb77-0727-4ce2-b558-31a96f3dbbb0
mean((out[1, :] .> .5) .== truth)

# ╔═╡ d4d2edac-cd1a-4955-983e-33ebdd100b1f
let
	out1 = untrained_model(noisy)
	out2 = out
...
# ╔═╡ cb38a7cf-f072-4e2b-9d46-b97356b5afb5
let
