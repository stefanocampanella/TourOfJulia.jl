...
begin
	...
	I will begin with some [preliminary stuff]($(root * "extra/prelims" * extension)) (installation, package management, editors and IDE support). After that, we will follow the first track and touch some topics taken from the second. Finally, we will see a few simple examples and applications, showing features and putting together a few things seen along the way.
	...
	1. [Built-in numerical types and missing values]($(root * "track 1/built-ins" * extension))
	1. [Variables]($(root * "track 1/variables" * extension))
	1. [Control flow]($(root * "track 1/controlflow" * extension))
	1. [Strings and multi-dimensional arrays]($(root * "track 1/arrays" * extension))
	...
	The examples have a varied degree of complexity and present the implementation of of both classical and numerical algorithms, how to tackle simple tasks relevant to statistics and Earth science, and showcase some popular libraries.
	...
	1. [Working with NetCDF]($(root * "examples/netcdf" * extension))
	1. [Working with tabular data]($(root * "examples/dataframes" * extension))
	1. [Learning XOR]($(root * "examples/xor" * extension))
...
On a less technical level, **writing Julia code is fun**[^3].
...
Julia is a **young programming language** (first public release in 2012, 1.0 release in 2018), and some tools and libraries might not be available. The package ecosystem is vast, but some packages might not have the maturity of the alternatives provided by other programming languages. 

Finally, when choosing the programming language for your next project, you should take into account the effort of learning a new language, the man-months spent on existing codebases, and the expertise and inclinations of the people you collaborate with. You should also consider that writing effective Julia code might require a deeper knowledge of the language than required by MATLAB or R.

So, _in which cases is Julia a good choice?_

One example is when you have to write **new algorithms from scratch**, they cannot be written as array operations (i.e. you are forced to use explicit loops), and you want to avoid the clumsiness of verbose, low-level languages without loosing efficiency. 

Another scenario might be **data-model integration using machine learning**: chances are that there will exist more stable or feature-complete models written in Fortran, and the de facto standard for ML is Python, but writing an interface between the two is non-trivial. Julia solves the issue by having a single language for the two purposes.
"""
